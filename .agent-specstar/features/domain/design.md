...
### Recommended approach: `shopspring/decimal`

**This library is NOT in the pre-approved list and requires explicit approval before implementation.**

`github.com/shopspring/decimal` is the de facto standard for arbitrary-precision decimal arithmetic in Go financial systems. It maps cleanly to PostgreSQL `NUMERIC` via string-based scanning, preserves exact scale, and provides correct arithmetic operations.

Rationale for recommending it over standard library alternatives:

- **`int64` fixed-point**: Multiplying `amount` (scale 1e6) by `unit_price` (scale 1e8) produces an intermediate value requiring 14 decimal digits of scale — this overflows `int64` for any non-trivial position sizes. Compensating with `math/big.Int` for intermediate calculations is possible but produces verbose, error-prone code for every arithmetic operation.
- **`math/big.Rat` / `math/big.Float`**: Standard library, but the API is cumbersome for common decimal operations (comparison, rounding to scale, database scanning). Every operation requires explicit result allocation. No built-in scale tracking.

### Standard-library-only fallback

If `shopspring/decimal` is not approved: use `int64` fixed-point representation with `math/big.Int` for multiplication and division operations that risk overflow. Each decimal field carries a compile-time-known scale constant. This approach is correct but significantly more verbose and error-prone.
...
- No migration framework. Migrations are applied by an external runner or a minimal bootstrap script.
...
| `github.com/shopspring/decimal`    | **Needs approval** (see Open Decisions) |
| `go.opentelemetry.io/otel`        | Optional, only if enabled by task     |
| Testing helper libraries           | Optional, only if enabled by task     |

## Open Decisions

1. **Decimal library approval**: `shopspring/decimal` is recommended for exact decimal arithmetic but is not in the pre-approved library list. Either approve it or confirm the `int64` fixed-point + `math/big` fallback approach. This decision blocks entity implementation.

2. **UUID generation strategy**: The schema uses UUID primary keys. Decide whether UUIDs are generated by the application (e.g., `google/uuid` — also needs approval) or passed in by callers. If application-generated, the UUID library needs to be added to the allowed list.

3. **Migration runner**: The design specifies plain SQL migration files with no framework. Clarify whether migrations are applied by an external CI/CD tool, a lightweight Go bootstrap utility, or manual execution. This affects whether any migration tooling code is in scope.

4. **`response_snapshot` schema**: The ProcessedCommand stores a JSONB snapshot. Clarify whether the snapshot structure will be defined per command type (typed) or kept as opaque JSON. This affects how the domain models the field (raw `[]byte` vs typed struct).
